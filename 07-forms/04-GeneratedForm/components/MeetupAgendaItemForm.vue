<template>
  <fieldset class="agenda-item-form">
    ...
    <ui-form-group v-for="item in itemSchema" :key="item" :label="item.label">
      <component :is="item.component" v-bind="item.props" v-model="localAgendaItem[item.props.name]"></component>
    </ui-form-group>
  </fieldset>
</template>
...
export default {
  ...
  watch: {
    startsAt(newValue, oldValue) {
      let newStartTime = this.timeToMilliseconds(newValue);
      let oldStartTime = this.timeToMilliseconds(oldValue);
      let deltaStartTime = newStartTime - oldStartTime;
      let oldEndTime = this.timeToMilliseconds(this.localAgendaItem.endsAt);
      this.localAgendaItem.endsAt = new Date(oldEndTime + deltaStartTime).toISOString().substr(11, 5);
    },
  ...
  methods: {
    timeToMilliseconds(time) {
      time = time.split(':');
      let hoursInMs = time[0] * 3600 * 1000;
      let minutesInMs = time[1] * 60 * 1000;
